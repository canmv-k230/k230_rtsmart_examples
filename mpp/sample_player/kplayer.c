...
static k_s32 _init_mp4_demuxer(const k_char *filePath)
{
    ...
    g_progress_info.total_time = file_info.duration / 1000; /* us -> ms, disp_play()/vdec expect ms */
...
static k_s64 _now_ms()
{
    struct timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);
    return (k_s64)ts.tv_sec * 1000 + ts.tv_nsec / 1000000;
}
...
static void *play_thread(void *arg)
{
    ...
    k_s64 start_system_time_ms = _now_ms();
    k_s64 paused_time_ms = 0;
    k_u64 start_video_timestamp_ms = 0;
    k_bool first_video_frame = K_TRUE;

    while (g_play_start)
    {
        if (g_play_pause)
        {
            k_s64 pause_start_ms = _now_ms();
            while (g_play_pause && g_play_start)
            {
                usleep(100*1000);
            }
            paused_time_ms += _now_ms() - pause_start_ms;
            continue;
        }
...
// printf("video data size:%d\n",frame_data.data_length);
                ...
                {
                    k_u64 video_timestamp_ms = frame_data.time_stamp / 1000;
                    if (first_video_frame)
                    {
                        start_video_timestamp_ms = video_timestamp_ms;
                        first_video_frame = K_FALSE;
                    }

                    /* pace playback to the video pts, otherwise frames are pushed to
                     * VDEC/AO as fast as they can be demuxed and their VB pools starve. */
                    k_u64 video_elapsed_ms = video_timestamp_ms - start_video_timestamp_ms;
                    k_s64 system_elapsed_ms = _now_ms() - start_system_time_ms - paused_time_ms;
                    if ((k_s64)video_elapsed_ms > system_elapsed_ms)
                    {
                        usleep((video_elapsed_ms - system_elapsed_ms) * 1000);
                    }

                    disp_play(frame_data.data, frame_data.data_length, video_timestamp_ms, K_FALSE);
                    g_progress_info.cur_time = video_timestamp_ms;
        ...
        else if (frame_data.codec_id == K_MP4_CODEC_ID_G711A || frame_data.codec_id == K_MP4_CODEC_ID_G711U)
        {
            if (g_audio_track != INVALID_STREAM_TRACK)
            {
                ...
                ao_play(frame_data.data,frame_data.data_length,frame_data.time_stamp / 1000);
            }
        }
    }
