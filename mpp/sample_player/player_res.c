...
static k_s32 _ao_vb_create_pool(k_u32 blk_size)
{
    k_vb_pool_config pool_config;
    memset(&pool_config, 0, sizeof(pool_config));
    ...
    printf("audio_pool_id %d\n", g_audio_pool_id);

    return (g_audio_pool_id == (k_u32)VB_INVALID_POOLID) ? K_FAILED : K_SUCCESS;
}
...
k_s32 ao_open(k_s32 s32SampleRate, k_s32 s32ChanNum,k_payload_type audio_dec_type, k_bool avsync)
{
    ...
    if (K_SUCCESS != _ao_vb_create_pool(g_enc_frame_len))
    {
    ...
    if (K_SUCCESS != kd_sample_sys_get_vb_block_from_pool_id(g_audio_pool_id, &g_audio_stream.phys_addr, g_enc_frame_len, NULL))
    {
        kd_mpi_vb_destory_pool(g_audio_pool_id);
        kd_sample_adec_unbind_ao(g_ao_dev,g_ao_chn,g_adec_hdl);
        kd_mpi_adec_destroy_chn(g_adec_hdl);
        kd_sample_ao_stop(g_ao_dev,g_ao_chn);
        return K_FAILED;
