...
BYTETracker::BYTETracker(float track_thresh_,float high_thresh_,float match_thresh_,int frame_rate, int track_buffer)
{
	// track_thresh = 0.4;
	// high_thresh = 0.5;
	// match_thresh = 0.7;
	...
	frame_id = 0;
	max_time_lost = int(frame_rate / 30.0 * track_buffer*2);
}
...
vector<STrack> BYTETracker::update(const vector<Object>& objects)
{
	...
	/*********************** Step 6: Update dwell time ************************/
	double now = get_timestamp_s();
	for (auto &track : this->tracked_stracks)
	{
		int id = track.track_id;
		if (track_times.find(id) == track_times.end())
		{
			// 第一次出现
			track_times[id] = {now, now};
		}
		else
		{
			// 更新最后一次时间
			track_times[id].last_seen = now;

			// 实时计算驻留时间（秒）
			double dwell_time = track_times[id].last_seen - track_times[id].first_seen;

			track.track_time = dwell_time;
		}
	}

	/*********************** Step 7: Handle removed tracks ************************/
	for (auto &track : removed_stracks)
	{
		int id = track.track_id;
		auto it = track_times.find(id);
		if (it != track_times.end())
		{
			double dwell_time = it->second.last_seen - it->second.first_seen;
			track_times.erase(it);
		}
	}
...
Scalar BYTETracker::get_color(int idx)
{
	...
	return Scalar(255,10 * idx % 255, 100 * idx % 255, 50 * idx % 255);
}
